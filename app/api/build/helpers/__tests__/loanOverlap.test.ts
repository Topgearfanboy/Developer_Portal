...
describe("getLoanOverlapMonths", () => {
  ...
  it.concurrent("should return 0 when loan start index is invalid", () => {
    const result = getLoanOverlapMonths(1, mockTimeline, -1, null);
    expect(result).toBe(0);
  });

  it.concurrent("should return 0 when loan end date is null", () => {
    const result = getLoanOverlapMonths(1, mockTimeline, 0, null);
    expect(result).toBe(0);
  });

  it.concurrent("should return 0 when renovate block does not exist", () => {
    const result = getLoanOverlapMonths(
      99,
      mockTimeline,
      0,
      new Date(2025, 0, 1),
  ...
  it.concurrent(
    "should calculate overlap when renovation is within loan period",
    () => {
  ...
  it.concurrent(
    "should return 0 when renovation ends before loan starts",
    () => {
  ...
  it.concurrent(
    "should return 0 when renovation starts after loan ends",
    () => {
      const timeline: TimelineEntry[] = [
        {
          type: "buy",
  ...
  it.concurrent("should calculate partial overlap", () => {
    const timeline: TimelineEntry[] = [
      {
        type: "buy",
        startDate: new Date(2024, 0, 1),
